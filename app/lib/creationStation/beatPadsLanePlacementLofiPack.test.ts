...
describe('Beat Pads Lo-Fi lane placements', () => {
  it('ships 20 of each kick / snare / clap / hihat / openHat / rim', () => {
    const roles = ['kick', 'snare', 'clap', 'hihat', 'openHat', 'rim'] as const;
    ...
    expect(new Set(BEAT_PADS_LOFI_LANE_TEMPLATES.map((t) => t.id)).size).toBe(120);
  });

  it('maps roles to canonical Beat Pads lanes', () => {
    expect(BEAT_PADS_CANONICAL_LANE_FOR_ROLE.kick).toBe(0);
    ...
    expect(BEAT_PADS_CANONICAL_LANE_FOR_ROLE.rim).toBe(7);
  });
});
