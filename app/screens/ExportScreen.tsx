...
export default function ExportScreen() {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: '#16161c', color: '#ccc' }}>
      <div className="flex items-center gap-3 px-5 py-3 shrink-0" style={{ borderBottom: '1px solid #0d0d14', background: '#0d0d14' }}>
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: '#7a7a88' }}>Export WAV, MP3 or MIDI — pattern, full song, or stems</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 min-h-0 flex flex-col gap-5">
        ...
        <div className="rounded-xl p-4 flex flex-col gap-3" style={{ background: '#0d0d14', border: '1px solid #1a1a24' }}>
          ...
          <div className="rounded-lg overflow-hidden flex items-center justify-center" style={{ height: 72, background: '#0d0d14', border: '1px solid #121218' }}>
            <div className="flex items-end gap-px w-full h-full px-4">
              {Array.from({ length: 120 }, (_, i) => (
                <div key={i} className="flex-1 rounded-full"
                  style={{ height: `${15 + Math.abs(Math.sin(i * 0.18) * 65 + Math.sin(i * 0.07) * 25)}%`, background: previewing ? '#00ff88' : '#0d0d14', transition: 'background 0.3s', opacity: 0.7 + Math.sin(i * 0.3) * 0.3 }} />
              ))}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ background: '#0d0d14' }}>
              <div className="h-full rounded-full" style={{ width: `${previewProgress}%`, background: '#00ff88', transition: 'width 0.1s linear' }} />
            </div>
            <span className="text-xs font-mono" style={{ color: '#7a7a88' }}>
            ...
            <div className="flex flex-col gap-1.5">
              {FORMATS.map(f => (
                <button key={f} onClick={() => setFormat(f)} className="py-2 rounded text-xs font-bold text-left px-3"
                  style={{ background: format === f ? '#00E5FF22' : '#121218', color: format === f ? '#00E5FF' : '#555', border: `1px solid ${format === f ? '#00E5FF' : '#1a1a24'}` }}>
                  {f}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              {QUALITIES.map(q => (
                <button key={q} onClick={() => setQuality(q)} className="py-2 rounded text-xs font-bold text-left px-3"
                  style={{ background: quality === q ? '#D500F922' : '#121218', color: quality === q ? '#D500F9' : '#555', border: `1px solid ${quality === q ? '#D500F9' : '#1a1a24'}` }}>
                  {q}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              {SCOPES.map(s => (
                <button key={s.id} onClick={() => setScope(s.id)} className="py-2 rounded text-xs font-bold text-left px-3 flex flex-col gap-0.5"
                  style={{ background: scope === s.id ? '#00ff8822' : '#121218', color: scope === s.id ? '#00ff88' : '#555', border: `1px solid ${scope === s.id ? '#00ff88' : '#1a1a24'}` }}>
                  <span className="flex items-center gap-1">{s.icon} {s.label}</span>
                  <span className="text-xs font-normal" style={{ color: scope === s.id ? '#00ff8888' : '#1a1a24', fontSize: 9 }}>{s.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-xl p-4 flex flex-col gap-3" style={{ background: '#0d0d14', border: '1px solid #1a1a24' }}>
          ...
          <div className="flex flex-wrap gap-1.5">
            <button onClick={() => setTargetIsle(null)}
              ...
              style={{ background: targetIsle === null ? '#0d0d14' : '#121218', color: targetIsle === null ? '#fff' : '#555', border: `1px solid ${targetIsle === null ? '#555' : '#1a1a24'}` }}>
              None (file only)
            </button>
            {sections.map(sec => (
              <button key={sec.id} onClick={() => setTargetIsle(sec.id)}
                className="px-2 py-1 rounded text-xs font-bold"
                style={{ background: targetIsle === sec.id ? `${sec.color}22` : '#121218', color: targetIsle === sec.id ? sec.color : '#555', border: `1px solid ${targetIsle === sec.id ? sec.color : '#1a1a24'}` }}>
                {sec.name}
              </button>
            ))}
          </div>
        ...
        <div className="rounded-xl p-4 flex flex-col gap-3" style={{ background: '#0d0d14', border: '1px solid #1a1a24' }}>
          <div className="flex items-center justify-between text-xs" style={{ color: '#7a7a88' }}>
          ...
          <button onClick={startExport} disabled={exporting || done}
            ...
            style={{ background: done ? '#00ff8822' : exporting ? '#0d0d14' : 'linear-gradient(135deg,#00ff88,#00E5FF)', color: done ? '#00ff88' : exporting ? '#444' : '#000', cursor: exporting || done ? 'not-allowed' : 'pointer', border: done ? '1px solid #00ff8844' : 'none' }}>
            {done ? '✓ Export Complete' : exporting ? 'Exporting…' : <><Download size={14} /> Export {SCOPES.find(s=>s.id===scope)?.label}</>}
          </button>

          {(exporting || done) && (
            <div>
              <div className="w-full h-2 rounded-full overflow-hidden mb-2" style={{ background: '#121218' }}>
